    ...
    (
        ...
        if (echo $dir | grep -qE $BUILD_SKIP_REGEX); then
            ...
            if (echo $dir | grep -qE $CROSS_BUILD_SKIP_REGEX); then
                ...
                make >./build.log 2>&1 && build_status="OK" || build_status="FAIL"
            fi
...
printf "%-64.64s%-12.12s%-12.12s\n" "Directory" "Build" "Test"
printf '=%.0s' {1..60}
printf '\n'

for f in $(find $LIMIT -name "Makefile" | sort ); do
    dir=$(dirname $f)
    printf "%-64.64s" $(echo $dir | sed s,$ROOT/*,,)
